...
/* Private user code ---------------------------------------------------------*/
/* USER CODE BEGIN 0 */
// redirect printf to USART2 (STLink Virtual COM), output flushed on newline \n
...
static void MX_RTC_Init(void)
{
  ...
  */
  ...
  // check if backup register still contains magic number
  // if not, reset the RTC to the calendar configured in .ioc
  if (HAL_RTCEx_BKUPRead(&hrtc, RTC_BKP_DR0) != TM_RTC_MAGIC)
  {
  ...
  */
  ...
  HAL_RTCEx_BKUPWrite(&hrtc, RTC_BKP_DR0, TM_RTC_MAGIC);
  }
...
static void MX_SDIO_SD_Init(void)
{
  ...
  // SD initialization should happen in 1 bit mode (bug in CubeMX)
...
/* USER CODE END MX_GPIO_Init_1 */
  ...
  HAL_GPIO_WritePin(GPIOA, LED_HEARTBEAT_Pin|LED_FAULT_Pin|LED_STATUS_Pin, GPIO_PIN_RESET);
...
/* USER CODE BEGIN Header_tm_task_service_can */
/**
* @brief Function implementing the ServiceCAN thread.
* @param argument: Not used
* @retval None
*/
/* USER CODE END Header_tm_task_service_can */
void tm_task_service_can(void const * argument)
{
  /* USER CODE BEGIN tm_task_service_can */
  /* Infinite loop */
  for(;;)
  {
    tm_service_can();
